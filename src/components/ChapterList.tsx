  ...
  return (
    <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-8">
      ...
      <div className="text-center mb-6 sm:mb-8 animate-fadeIn">
        <h1 className="font-serif text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-maroon mb-2">
          {textNameHindi}
        </h1>
        <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-ink/70 mb-4">
          {textName}
        </p>
        {hasPDF && onViewPDF && (
          <button
            onClick={onViewPDF}
            className="inline-flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-saffron text-white rounded-lg font-medium hover:bg-saffron-dark active:bg-saffron-dark transition-all duration-200 shadow-md hover:shadow-lg touch-manipulation"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 max-w-7xl mx-auto">
        {filteredChapters.map((chapter, index) => (
          <div
            ...
            className="folio lotus-corner rounded-xl hover:shadow-2xl active:scale-95 transition-all duration-300 cursor-pointer touch-manipulation p-4 sm:p-6 animate-fadeIn"
            style={{ animationDelay: `${index * 0.05}s` }}
          >
            <div className="text-center">
              <div className="text-2xl sm:text-3xl font-bold text-saffron mb-2">
                {chapter.chapter_number}
              </div>
              <h3 className="font-serif text-base sm:text-lg font-semibold text-maroon mb-2">{chapter.name}</h3>
              <p className="text-sm text-ink/70 mb-2 sm:mb-3">{chapter.name_meaning}</p>
              <p className="text-xs text-ink/50 mb-2 line-clamp-2">{chapter.summary}</p>
